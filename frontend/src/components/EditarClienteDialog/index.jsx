...
export default function EditarClienteDialog({ open, cliente, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (cliente) {
      // Converter ramos_atividade para cnaes se necessário
      if (cliente.ramos_atividade && !cliente.cnaes) {
        const cnaesConvertidos = cliente.ramos_atividade.map((ramo, index) => ({
          codigo: `00.00-0-${index.toString().padStart(2, '0')}`,
          descricao: ramo,
