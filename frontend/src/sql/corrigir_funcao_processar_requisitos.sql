...
BEGIN
    ...
    -- Obter a ordem máxima atual para continuar a partir dela
    SELECT COALESCE(MAX(ordem), 0) INTO v_max_ordem FROM requisitos_documentacao WHERE licitacao_id = p_licitacao_id;
    v_contador := v_max_ordem + 1;

    -- NÃO excluímos mais os requisitos existentes
    -- Apenas adicionamos novos requisitos
               ...
               v_linha ~* '(ME|EPP|Microempresa|Empresa de Pequeno Porte).*dever[áã]' THEN  -- Requisitos específicos ME/EPP
                ...
                ELSE
                    ...
                    IF length(v_texto_requisito) > 0 THEN
                        ...
                        -- Verificar se o requisito já existe (para evitar duplicatas)
                        IF NOT EXISTS (
                            SELECT 1 FROM requisitos_documentacao 
                            WHERE licitacao_id = p_licitacao_id 
                            AND descricao = trim(v_texto_requisito)
                        ) THEN
    ...
    -- Se nenhum requisito foi encontrado e não há requisitos existentes, adicionar uma mensagem
    IF v_contador = v_max_ordem + 1 AND v_max_ordem = 0 THEN
        WITH inserted AS (
            INSERT INTO requisitos_documentacao (
                licitacao_id,
                descricao,
